...
* **A Git Repository Namespace on the Source:** You will need a Git Repository namespace on the source cluster, which you will migrate from the source cluster to the destination using AsyncDR. You can create a new git-repo with the help of [this page](../gitscm-server/README.md).
...
* Make some changes and push to the central repo:
		...
		git push central (dev branch name)

 * **If failed to push to main  repository**

		git pull central (dev branch name)
		git rebase central/(dev branch name)
		git push central (dev branch name)
