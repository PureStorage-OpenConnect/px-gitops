echo > ../argo-worflow/java-app/px-snaphost-restore-script/px-restore-snapshot.yaml
echo > ../argo-worflow/java-app/px-snaphost-restore-script/create-snapshot.sh
echo > ../argo-worflow/java-app/px-snaphost-restore-script/restore-snapshot.sh
echo > ../argo-worflow/java-app/px-snaphost-restore-script/px-snapshot.yaml
...
cp ../argo-worflow/java-app/px-snaphshot-restore-script-template/px-restore-snapshot.yaml ../argo-worflow/java-app/px-snaphost-restore-script/px-restore-snapshot.yaml
cp ../argo-worflow/java-app/px-snaphshot-restore-script-template/create-snapshot.sh   ../argo-worflow/java-app/px-snaphost-restore-script/create-snapshot.sh
cp ../argo-worflow/java-app/px-snaphshot-restore-script-template/restore-snapshot.sh  ../argo-worflow/java-app/px-snaphost-restore-script/restore-snapshot.sh
cp ../argo-worflow/java-app/px-snaphshot-restore-script-template/px-snapshot.yaml  ../argo-worflow/java-app/px-snaphost-restore-script/px-snapshot.yaml
...
echo "                                                    "
sed -ie "s,XX-namespace-XX,$PX_Application_MainBranch_Namespace,g" ../argo-worflow/java-app/px-snaphost-restore-script/px-restore-snapshot.yaml
sed -ie "s,XX-repo-XX,$REPONAME,g" ../argo-worflow/java-app/px-snaphost-restore-script/px-snapshot.yaml
sed -ie "s,XX-namespace-XX,$PX_Application_MainBranch_Namespace,g"  ../argo-worflow/java-app/px-snaphost-restore-script/create-snapshot.sh
sed -ie "s,XX-namespace-XX,$PX_Application_MainBranch_Namespace,g" ../argo-worflow/java-app/px-snaphost-restore-script/restore-snapshot.sh
sed -ie "s,XX-namespace-XX,$PX_Application_MainBranch_Namespace,g" ../argo-worflow/java-app/px-snaphost-restore-script/px-snapshot.yaml
sleep 2



echo "Creating kubernetes px snapshot script secret for main branch  "
kubectl create secret generic java-snapshot-script-main-branch --from-file=../argo-worflow/java-app/px-snaphost-restore-script/px-snapshot.yaml  --from-file=../argo-worflow/java-app/px-snaphost-restore-script/create-snapshot.sh -n argo
sleep 2
echo "                                                    "
echo "                                                    "
echo "Creating kubernetes px restore snapshot script secret for main branch "
kubectl create secret generic java-restore-snapshot-script-main-branch  --from-file=../argo-worflow/java-app/px-snaphost-restore-script/px-restore-snapshot.yaml  --from-file=../argo-worflow/java-app/px-snaphost-restore-script/restore-snapshot.sh -n argo
sleep 2
echo "                                           "
...
sleep 5
sed -ie "s,XX-namespace-XX,$PX_Application_DevBranch_Namespace,g" ../argo-worflow/java-app/px-snaphost-restore-script/px-restore-snapshot.yaml
sed -ie "s,XX-repo-XX,$REPONAME,g" ../argo-worflow/java-app/px-snaphost-restore-script/px-snapshot.yaml
sed -ie "s,XX-namespace-XX,$PX_Application_DevBranch_Namespace,g"  ../argo-worflow/java-app/px-snaphost-restore-script/create-snapshot.sh
...
kubectl create secret generic java-snapshot-script-dev-branch --from-file=../argo-worflow/java-app/px-snaphost-restore-script/px-snapshot.yaml  --from-file=../argo-worflow/java-app/px-snaphost-restore-script/create-snapshot.sh -n argo
sleep 2
echo "                                                    "
echo "                                                    "
echo "Creating kubernetes px restore snapshot script secret for dev branch "
kubectl create secret generic java-restore-snapshot-script-dev-branch  --from-file=../argo-worflow/java-app/px-snaphost-restore-script/px-restore-snapshot.yaml  --from-file=../argo-worflow/java-app/px-snaphost-restore-script/restore-snapshot.sh -n argo
sleep 2
echo "                                           "
kubectl create secret generic java-dev-branch-cluster-kube-config --from-file=config -n argo

    countYaml=`ls -1 ../argo-worflow/java-app/px-snaphost-restore-script/*.yamle 2>/dev/null | wc -l`
    countScripts=`ls -1 ../argo-worflow/java-app/px-snaphost-restore-script/*.she 2>/dev/null | wc -l`
    if [ "$countYaml" != "0" ] && [ "$countScripts" != "0" ]
    then 
    rm ../argo-worflow/java-app/px-snaphost-restore-script/*.yamle
    rm ../argo-worflow/java-app/px-snaphost-restore-script/*.she
    fi
