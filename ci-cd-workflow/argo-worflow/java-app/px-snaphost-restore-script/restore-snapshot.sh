...
vSNAPSHOT="$(kubectl get VolumeSnapshot -n springboot-code-main -l=name=snapshot-of-java-app --sort-by=.metadata.creationTimestamp -o jsonpath='{.items[-1:].metadata.name}')"
echo $vSNAPSHOT
