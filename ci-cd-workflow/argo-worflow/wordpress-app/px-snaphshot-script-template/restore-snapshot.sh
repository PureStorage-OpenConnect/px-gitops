...
if [[ "$Noresource" == "$getSnapshotRestore" ]]; then
    ...
    vSnapshotRestore="$(kubectl get VolumeSnapshotRestore -n XX-namespace-XX -l=name=restore-of-wordpress-app  --sort-by=.metadata.creationTimestamp -o jsonpath='{.items[-1:].metadata.name}')"
