...
##Checking S3-Bucket credentials:
  ...
  if [[ "${PX_S3_BUCKET_ENDPOINT}x" == "x" ]]; then    echo "PX_S3_BUCKET_ENDPOINT is not set. Please edit the ${vCONFIGFILE} file and put the correct value."; exit 1; fi
  if [[ "${PX_S3_DISABLE_SSL}x" == "x" ]]; then        echo "PX_S3_DISABLE_SSL is not set. Please edit the ${vCONFIGFILE} file and put the correct value."; exit 1; fi
...
##Setting kube-configs.
  ...
  if [[ "${PX_KUBECONF_PATH_SOURCE_CLUSTER}x" != "x" ]]; then
    ...
    echo -e "\nSelecting default cluster as source.\nTo use a different cluster edit the ${vCONFIGFILE} file and set the PX_KUBECONF_PATH_SOURCE_CLUSTER variable."
  fi
  if [[ "${PX_KUBECONF_PATH_DESTINATION_CLUSTER}x" != "x" ]]; then
    PX_KUBECONF_DESTINATION="--kubeconfig=${PX_KUBECONF_PATH_DESTINATION_CLUSTER}";
  else
    echo -e "\nSelecting default cluster as destination.\nTo use a different cluster edit the ${vCONFIGFILE} file and set the PX_KUBECONF_PATH_DESTINATION_CLUSTER variable."
  fi
...
##Prepare backup location and backup manifests:
  ...
  sed "s,XX_ACCESS_KEYID_XX,${PX_S3_ACCESS_KEY_ID},g" | \
  sed "s,XX_SECRET_ACCESS_KEY_XX,${PX_S3_SECRET_ACCESS_KEY},g" | \
  sed "s,XX_BUCKET_ENDPOINT_XX,${PX_S3_BUCKET_ENDPOINT},g" | \
  sed "s,XX_DISABLE_SSL_XX,${PX_S3_DISABLE_SSL},g" | \
...
##Start taking backup
  ...
  while (( vChecksDone <= vTotalChecks ))
    do
      ...
      vStatus="$( echo "${vRetVal}"|awk -F ' +' '{print $1}' | tr '[:upper:]' '[:lower:]')"
      vState="$(  echo "${vRetVal}"|awk -F ' +' '{print $2}' | tr '[:upper:]' '[:lower:]')"
      ...
      if [[ "${vState}" = "final" ]]; then
        if [[ "${vStatus}" = "successful" ]]; then
          ...
          vConfirm="$(echo ${vConfirm} | tr '[:upper:]' '[:lower:]')"
          if [[ "${vConfirm}" != "y" ]]; then
...
##Cleanup
  ...
  PX_KEEP_BACKUP="$(echo ${PX_KEEP_BACKUP} | tr '[:upper:]' '[:lower:]')"
  if [[ "${PX_KEEP_BACKUP}" != "yes" ]]; then
    if [[ "${PX_KUBECONF_PATH_SOURCE_CLUSTER}" = "${PX_KUBECONF_PATH_DESTINATION_CLUSTER}" ]]; then
...
PX_GIT_SERVICE_MANIFEST_FILE=${GENERATED_MANIFESTS_DIR}/git-service.yaml;

cat "${TEMPLATES_MANIFEST_DIR}/service.yaml" | \
sed "s,XX-namespace-XX,$PX_DESTINATION_NAMESPACE,g" > "${PX_GIT_SERVICE_MANIFEST_FILE}"

kubectl apply -f "${PX_GIT_SERVICE_MANIFEST_FILE}" ${PX_KUBECONF_DESTINATION}
...
echo -e "\nChecking pod status.....";  
  ...
  while (( vChecksDone <= vTotalChecks ))
    do  
      vRetVal="$(kubectl get pod -n $PX_DESTINATION_NAMESPACE ${PX_KUBECONF_DESTINATION} | awk 'FNR==2{print $3}')"
